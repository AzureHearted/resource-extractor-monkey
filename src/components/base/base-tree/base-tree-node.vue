...
import NodeLabel from "./base-tree-node-label.vue";

import { useCustomDrag } from "@/hooks/useBaseCustomDrag";
import type { provideType } from "./base-tree.vue";
...
const defaultSlot = computed(() => {
	if (!hasDefaultSlot.value) return null;
	const vNode = slots?.default({ nodeData: props.nodeData });
	if (vNode) {
		return {
			render: () => vNode,
		};
...
const allowShow = computed(() => {
	return globalState.value.filterKeys.includes(props.nodeData.id);
});
...
const {
	...
	dragging,
	init: initDrag,
} = useCustomDrag(() => nodeLabelRef.value?.$el, {
