...
import type { PatternRowData } from "../interface/Pattern";
...
export function getUserPatternList(): Pattern[] {
	let result = GM_getValue<PatternRowData[] | string>("UserPatternList", []);

	// 如果是 string 类型则通过 json 解析成对象数组
	if (typeof result === "string") {
		result = [...(JSON.parse(result) as Array<PatternRowData>)];
	}
